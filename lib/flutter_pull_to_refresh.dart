...
import 'package:flutter_pull_to_refresh/src/indicator/classic_indicator.dart';
import 'package:flutter_pull_to_refresh/src/indicator/material_indicator.dart';
import 'package:flutter_pull_to_refresh/src/internals/indicator_wrap.dart' as refresh;
import 'package:flutter_pull_to_refresh/src/internals/indicator_wrap.dart';
import 'package:flutter_pull_to_refresh/src/internals/refresh_physics.dart';
import 'package:flutter_pull_to_refresh/src/internals/slivers.dart';
...
class SmartRefresher extends StatefulWidget {
  ...
  /// Controller whether open the second floor function
  ...
  /// for example,NestedScrollView is a StatlessWidget,it's headerSliversbuilder can return a slivers array,So if we want to do
...
class SmartRefresherState extends State<SmartRefresher> {
  ...
  var defaultHeader = defaultTargetPlatform == TargetPlatform.iOS ? const ClassicHeader() : const MaterialClassicHeader();
  ...
  ScrollPhysics _getScrollPhysics(RefreshConfiguration? conf, ScrollPhysics physics) {
    final bool isBouncingPhysics = physics is BouncingScrollPhysics || (physics is AlwaysScrollableScrollPhysics && ScrollConfiguration.of(context).getScrollPhysics(context).runtimeType == BouncingScrollPhysics);
...
class RefreshController {
  ...
  StatefulElement? _findIndicator(BuildContext context, Type elementType) {
    ...
    context.visitChildElements((Element e) {
      if (elementType == material.RefreshIndicator) {
        if (e.widget is material.RefreshIndicator) {
          result = e as StatefulElement?;
        }
      } else {
        if (e.widget is refresh.LoadIndicator) {
          result = e as StatefulElement?;
        }
      }
  ...
  Future<void>? requestRefresh({bool needMove = true, bool needCallback = true, Duration duration = const Duration(milliseconds: 500), Curve curve = Curves.linear}) {
    ...
    StatefulElement? indicatorElement = _findIndicator(position!.context.storageContext, material.RefreshIndicator);

    if (indicatorElement == null || _refresherState == null) return null;
    (indicatorElement.state as material.RefreshIndicatorState);
        ...
        await position?.animateTo(position!.minScrollExtent - 0.0001, duration: duration, curve: curve).then((_) {
          if (_refresherState != null && _refresherState!.mounted) {
            ...
            if (needCallback) {
              ...
              if (indicatorElement.state.mounted) (indicatorElement.state as material.RefreshIndicatorState).setState(() {});
            }
          }
        });
      });
...
class RefreshConfiguration extends InheritedWidget {
  ...
  })  : assert(RefreshConfiguration.of(context) != null, "search RefreshConfiguration anscestor return null,please  Make sure that RefreshConfiguration is the ancestor of that element"),
